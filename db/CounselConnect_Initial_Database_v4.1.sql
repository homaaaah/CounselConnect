-- ============================================================
-- CounselConnect Initial Database Schema v4.1 (MySQL 8.4-compatible)
-- Target: MySQL 8.4 LTS
--
-- v4.1 CHANGES (v4 remains the approved design; only DDL-level
-- incompatibilities with MySQL 8.4 are corrected):
-- 1. chk_verifications_decision_shape REMOVED from DDL.
-- 2. chk_wellness_resources_provenance and
--    chk_wellness_resources_review_state REMOVED from DDL.
--    -> These three invariants are ENFORCED IN THE FASTAPI SERVICE
--       LAYER instead (as ARCHITECTURE.md already mandates).
--       MySQL 8 forbids CHECK constraints referencing columns that
--       participate in FK referential actions (ERROR 3823).
-- 3. fk_appointments_slot changed to ON UPDATE RESTRICT (from CASCADE):
--    availability_slot_id is the base column of a STORED generated
--    column; cascading updates into it are forbidden (ERROR 1215).
...
CREATE TABLE IF NOT EXISTS enrollment_verifications (
    ...
    -- v4.1: chk_verifications_decision_shape moved to the FastAPI
    -- enrollment_verification service layer (MySQL 8 ERROR 3823: CHECK may
    -- not reference a column used in an FK referential action).
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS appointments (
    ...
    CONSTRAINT fk_appointments_slot
        ...
        ON DELETE RESTRICT
        ON UPDATE RESTRICT, -- v4.1: base column of a STORED generated column
...
CREATE TABLE IF NOT EXISTS wellness_resources (
    ...
    -- chk_wellness_resources_review_state moved to the FastAPI
    -- wellness_resources service layer (MySQL 8 ERROR 3823).
) ENGINE=InnoDB;
