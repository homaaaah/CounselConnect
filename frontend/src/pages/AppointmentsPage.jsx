...
/**
 * AppointmentsPage — role-split appointment views (2026-09-13):
 *
 *  StudentAppointmentsPage: the student's own records — Confirmed /
 *    Pending / Completed tabs, cancel + reschedule (via BookingModal).
 *    PLACEHOLDER LABELS: every displayed field maps 1:1 to documented
 *    GET /appointments response fields (see docs/API_CONTRACT.md) so
 *    wiring the student-side data is a pure prop pass.
 *
 *  CounselorAppointmentsPage: student appointment records with review
 *    actions, plus an AVAILABILITY EDIT MODE — the counselor edits
 *    which days, times, and campus they are available. Editing a weekly
 *    schedule = POST a corrected schedule + DELETE the old one (the
 *    contract has no in-place update; both endpoints are documented).
 *    Clean placeholder rows exist for every field the student side will
 *    consume (campus, day, time, mode, slot length).
 *
 * Scheduling follows docs/API_CONTRACT.md: transitions are
 * server-authorized; nothing here can bypass a role.
 */
...
function RecordsTabs({ state, counselor }) {
...
function StudentAppointmentsPage({ user }) {
  // PLACEHOLDER-FRIENDLY: state.appointments.items maps directly to the
  // GET /appointments envelope; AppointmentCard reads only documented
  // response fields (starts_at, ends_at, campus_name, counselor_name,
  // appointment_mode, status, meeting_location, rejection_note,
...
/**
 * WeeklyScheduleEditor — the availability EDIT MODE (user request
 * 2026-09-13). The counselor edits their available days, times, and
 * campus in a form seeded with an existing schedule's values; saving
 * POSTs the corrected schedule and DELETES the old one, because the
 * documented contract has no in-place weekly-schedule update.
 * Rows are clean placeholders: one row per weekly schedule, showing
 * exactly the fields the student side consumes (campus, day, time,
 * mode, slot length).
 */
function WeeklyScheduleEditor({ state }) {
  ...
  // GET /weekly-schedules returns a list envelope {items, total, ...}.
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-6">
    ...
    {/* Editor form — creates new or replaces an existing definition */}
    ...
    {/* Availability rows — clean placeholders: one row per definition,
        exactly the fields the student-side calendar consumes. */}
...
function CounselorAppointmentsPage({ user }) {
  const state = useAppointments(user.role_code, "CONFIRMED");
  const [rescheduleId, setRescheduleId] = useState(null);
  return <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-800 sm:px-6">
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {/* Student records with review actions */}
      <section className="rounded-xl border border-slate-200 bg-white p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
